...
// —— 分组：每天每组各一个班次槽位 db.schedule[date][groupId] ——
...
function slotOf(db, date, groupId) {
  ...
  const e = day && day[groupId];
  return e && e.personId ? e : null;
}

function setSlot(db, date, groupId, entry) {
...
function clearPersonSlots(db, personId) {
  ...
  for (const [date, day] of Object.entries(db.schedule)) {
    ...
    for (const g of db.groups) {
      const e = day[g.id];
      if (e && e.personId === personId) {
        setSlot(db, date, g.id, null);
        cleared.push(date);
      }
    }
  }
...
function nextAssignee(db, beforeDate, groupId) {
  ...
  for (const [date, day] of Object.entries(db.schedule)) {
    const e = day && day[groupId];
    if (e && e.personId && date < beforeDate && (lastDate === null || date > lastDate)) lastDate = date;
  }
  if (lastDate === null) return people[0];
  const idx = people.findIndex((p) => p.id === db.schedule[lastDate][groupId].personId);
  return people[(idx + 1) % people.length];
}

// 生成一周排班（单组）：只填空缺的日期；该组该周已生成过则不动（保留人工弃班留空）
...
module.exports = {
  ...
  groupById, peopleOf, slotOf, setSlot, genWeeks, clearPersonSlots,
  nextAssignee, ensureWeekGenerated, ensureWeekAllGroups, entryHours, computeStats,
};
