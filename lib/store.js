...
function defaultConfig() {
  ...
  return {
    ...
    // —— 果果剧库联动（短剧账号自动注册；纯增量）——
    jukuBaseUrl: '',        // 剧库地址，如 http://服务器IP:8999；空=未启用联动与入口
    jukuAdminUser: '',      // 剧库管理员账号（排班站服务端代为建号）
    jukuAdminPassword: '',  // 剧库管理员密码（只存 data/db.json，接口永不回显）
    jukuBridgeSecret: '',   // 免登录桥共享密钥（与剧库启动参数 -bridge-secret 一致；空=成员需手动登录）
    // —— DeepSeek AI 夜间检查 ——
