...
function makePerson(name, password, groupId) {
...
function normalizeDb(db) {
  ...
  for (const id of ['g1', 'g2']) {
    if (!db.groups.find((g) => g.id === id)) db.groups.push({ id, name: id === 'g1' ? '一组' : '二组' });
  }
