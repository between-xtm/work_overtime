...
function makePerson(name, password) {
...
function newDb() {
  return {
    people: DEFAULT_PEOPLE.map((n) => makePerson(n, DEFAULT_USER_PASSWORD)),
    schedule: {},        // { 'YYYY-MM-DD': { personId, hours?, note? } }
    weeksGenerated: [],  // 已自动生成过的周（周一日期）；生成过的周里弃班留空、不会被回填
...
function normalizeDb(db) {
  ...
  if (!Array.isArray(db.weeksGenerated)) db.weeksGenerated = [];
  return db;
}
...
module.exports = {
  Store, hashPassword,
  DEFAULT_USER_PASSWORD, DEFAULT_ADMIN_PASSWORD, DATA_DIR,
};
